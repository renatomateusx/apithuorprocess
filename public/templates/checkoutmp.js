...
class CheckoutMP extends HTMLElement {
    ...
    async adicionarProdutoUpSell() {
        ...
        produtosCart.forEach((obj, i) => {
            ...
            const LTemplate = TEMPLATE_ITENS_CART.replace("{img}", obj.variant_img).replace("{title}", obj.title).replace("{variant_title}", obj.variant_title).replace("{quantity}", obj.quantity).replace("{variant_price}", this.formatPrice(obj.variant_price));
